...
# size; 1 day matches the value used in the full base_irradiation benchmark.
solver.set_step_size(86400.0)
solver.set_verbosity(4)

# ---------------------------------------------------------------------------
# Run: truncated depletion 0 -> 200 days (short test to check gas pressure
# behavior without waiting for the full 2176-day run), HDF5 output (C++ streaming)
# ---------------------------------------------------------------------------
D2S = 86400.0
TEND = 200 * D2S
DTOUT = 20  * D2S   # output every 20 days (10 steps over the 200 d test run)

THIS_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_H5 = os.path.join(THIS_DIR, "fred_m_na_test_200d.h5")
...
TARGET_DAYS = [0, 45, 135]
for col, tgt_day in enumerate(TARGET_DAYS):
    ax = axes[0, col]
    if tgt_day == 0:
        # FRED-M-Na starts from a cold (room-temperature) state at t=0, while
        # the legacy FRED-M/SAS4A "t=0" reference is already at hot full-power
        # steady state. Compare against the earliest available FRED-M-Na output
        # after reaching hot steady state but before significant burnup instead
        # of the literal cold-start point.
        nonzero = np.flatnonzero(times_d > 0)
        idx_mna = nonzero[0] if len(nonzero) else 0
    else:
    ...
    if HAS_REF and tgt_day in ref_data:
        ...
        if T_fi_fred and len(T_fi_fred) == NZ:
            ...
            err = T_fi_K[idx_mna] - T_fi_fred_K
