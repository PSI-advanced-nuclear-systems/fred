...
void FredMNaResiduals::computeThermalResiduals(int j, double t,
    ...
    m_heat.computeResiduals(s, T_cool, m_state.gpres, qqv, r_th, prescribed, h_cool_j);
    r_th[3] = s.hgap - computeMNaGapConductance(s, m_state.gpres, m_geom, m_gap_mat,
                                                  m_fuel, m_clad);
...
// one-step backward-Euler integrator's per-layer local Newton solve.
...
void FredMNaResiduals::computeLayerResiduals(int j, double t, const double* yj,
    ...
    if (isMechOn()) {
        std::vector<double>& r_mech = threadLocalMechMnaBuffer();
        m_mech_mna.computeResiduals(m_state.layers[j], m_state.gpres, pcool, r_mech);
