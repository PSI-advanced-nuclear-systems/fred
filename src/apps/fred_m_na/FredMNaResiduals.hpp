...
public:
    ...
    //     for ONE layer only (O(1), not O(nz) for the full rod), given that
    //     m_state's globals (gpres) and all OTHER layers are
    ...
    // time t. computeLayerResiduals is scoped to a single layer and does NOT
    // call this (that's the point of avoiding the O(nz) cost), so the
    // one-step integrator must call this itself once per outer Picard sweep,
    // or layerHTC()/the Robin outer-clad-surface BC silently reads a
    // stale/uninitialised coolant field.
...
private:
    ...
    // FRED-M-Na's own three-state mechanics (Decision D2/D8); used by
    // computeLayerResiduals, in place of the inherited (two-state) m_mech.
    // m_mech_mna itself holds no mutable state (safe to share across
    // threads); its residual output buffer must be thread-local (see
    // threadLocalThermalBuffer()'s doc comment in RodResiduals.hpp).
    FredMNaStressStrain m_mech_mna;
    std::vector<double>& threadLocalMechMnaBuffer() const {
        thread_local std::vector<double> buf;
        buf.resize(m_mech_mna.neq());
        return buf;
    }
