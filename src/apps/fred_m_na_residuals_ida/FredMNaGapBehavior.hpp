...
public:
    ...
    // Returns true if the gap-contact state changed this call (any flag
    // transition) -- informational only; FredMNaSolver's fixed-step
    // integrator carries no adaptive-step history, so no reinit is needed
    // when this fires (unlike FRED-ROD/FRED-OX's IDA-based gap events).
    static bool stateChanged(const std::string& flag_before,
                             const std::string& flag_after);
};
