...
public:
    ...
    void run(double tend, double dtout, bool all_steps = false);
...
protected:
    ...
    // Uses FredMNaResiduals's own residual entry point (not the shared
    // RodResiduals::idaResidual) so mechanics goes through FredMNaStressStrain
    // (three-state open/soft/clos BCs) instead of the inherited two-state
    // StressStrain — see FredMNaResiduals::computeResidualsMNa.
    IDAResFn getSolverResFn()                        const override { return FredMNaResiduals::idaResidualMNa; }
    ...
    void handleGapClosed   (int j)    override;
    void handleGapReopened (int j)    override;
...
private:
    ...
    // Replaces FredSolverBase::runTimeLoop / setupSundials for this app only
    // (see class comment). No SUNDIALS IDA integrator calls anywhere below.
    ...
    // discretisation of layer j's own neq_j-sized block, PLUS gpres solved
    // jointly as one extra unknown: gpres's own defining equation
    // (gpres = computeGasPressure(...)) has real cross-coupling to this
    // layer's mechanics (fuel/clad radii, axial strain), so solving it
    // together with the layer gives tighter within-step coupling than a
    // separate direct-substitution pass. yj/ypj/*gpres_inout updated in
    // place (gpres_inout is shared across all layers in a step — see
    // solveStepBackwardEuler); returns the infinity norm of the final
    // Newton correction (over the layer unknowns and the jointly-solved gpres).
    ...
    // Restart-state injection without touching IDA (applyRestartToIDA calls
    // IDAReInit, which requires m_ida_mem — never created in one-step mode).
