...
void bind_fred_m_na(py::module_& m) {
    ...
    py::class_<FredMNaSolver>(m, "FredMNaSolver")
        ...
        .def("set_tolerances",            &FredMNaSolver::setTolerances,         py::arg("rtol"), py::arg("atol"),
             "Unused by FRED-M-Na (no SUNDIALS IDA integrator); kept for API\n"
             "compatibility with FredSolverBase. Has no effect on this app's run().")
        .def("set_max_step",              &FredMNaSolver::setMaxStep,            py::arg("hmax"),
             "Unused by FRED-M-Na (no SUNDIALS IDA integrator; use set_step_size\n"
             "instead). Kept for API compatibility with FredSolverBase.")
        .def("set_init_step",             &FredMNaSolver::setInitStep,           py::arg("hinit"),
             "Unused by FRED-M-Na (no SUNDIALS IDA integrator). Kept for API\n"
             "compatibility with FredSolverBase.")
        .def("set_max_nonlin_iters",      &FredMNaSolver::setMaxNonlinIters,     py::arg("n"),
             "Unused by FRED-M-Na (no SUNDIALS IDA integrator; each axial layer's\n"
             "local Newton solve is capped internally). Kept for API compatibility\n"
             "with FredSolverBase.")
        .def("set_max_order",             &FredMNaSolver::setMaxOrd,             py::arg("order"),
             "Unused by FRED-M-Na (no SUNDIALS IDA integrator — the one-step\n"
             "integrator is backward Euler unconditionally, matching legacy FRED-M's\n"
             "fixed-step scheme; no BDF order to select). Kept for API compatibility\n"
             "with FredSolverBase.")
